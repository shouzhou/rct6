                                 ...
                                 size_t          max_buffer_len )
{
  uint16_t           code=0;
	...
	ack=strstr((const char *)buffer, "+MIPLEVENT:0,26");
	reg_rsp=strstr((const char *)buffer, "+MIPLEVENT:0,6");
	if(reg_rsp==NULL)
	  reg_rsp=strstr((const char *)buffer, "+MIPLEVENT:0,7");
	close_rsp=strstr((const char *)buffer, "+MIPLEVENT:0,15");
	update_rsp=strstr((const char *)buffer, "+MIPLEVENT:0,11");
	if(read!=NULL)
       code=COAP_READ;
	else if(write!=NULL)
       code=COAP_WRITE;
...
void nbiot_object_add( nbiot_device_t *dev)
{
   ...
   uint16_t resid[20],rescont=0,rescount1=0;
   char tmp[10];
   char buf[512],i=0;
   ...
   while(obj!=NULL)
   {
	 ...
	 instptr[inscount]='\0';
	 m5310_addobj(obj->id,inscount,instptr,0,0);
	 buf[0]='\"';
	 for(i=0;i<rescont;i++)
	 {
       ...
       buf[len++]=*msg++;
	     buf[len]='\0';
	  	...
	  	if(*msg=='\0')
		     trigger=0;
        if(updated==true){
          m5310_notify_upload(&uri1,type,buf,trigger,0,msgid);
					if(trigger==1)
						trigger=2;
		   }else{
		     m5310_read_upload(&uri1,type,buf,msgid,1,0,trigger);
				 if(trigger==1)
						trigger=2;
			}		
		  if(0==trigger)
