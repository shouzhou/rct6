...
/* 定义GPIO端口 */
#define RCC_DQ		RCC_APB2Periph_GPIOA
...
//	}
	...
	if (sum == _pDTH->Buf[4])
	{
		_pDTH->Temp = _pDTH->Buf[2]<<8 | _pDTH->Buf[3];	/* 温度整数部分 */
		_pDTH->Hum  = _pDTH->Buf[0]<<8 | _pDTH->Buf[1];	/* 湿度整数部分 */
		return 1;
	}
quit:
	return 0;
...
static uint8_t DHT11_ReadByte(void)
{
	...
	*/
	...
	for (i = 0; i < 8; i++)
	{
        ...
        if(!DQ_IS_LOW()) read++;
       // else  read =1 ;
//		if (k > 4)		/* 高脉冲持续时间大于40us ，认为是1，否则是0 */
//		{
//			read++;
//		}
	}
