...
float FunO3CalParameterKV(float  _allV)
{
    if(_allV>=KVLEVEL1)
        ...
        kv = KVL8;
		else if(_allV>=KVLEVEL9)
        kv = KVL9;
		else if(_allV>=KVLEVEL10)
        kv = KVL10;
		else if(_allV>=KVLEVEL11)
        kv = KVL11;
		else if(_allV>=KVLEVEL12)
        kv = KVL12;
		else if(_allV>=KVLEVEL13)
        kv = KVL13;
		else if(_allV>=KVLEVEL14)
        kv = KVL14;
		else if(_allV>=KVLEVEL15)
        kv = KVL15;
		else if(_allV>=KVLEVEL16)
        kv = KVL16;
		    else 
        kv = KVOTHER;
...
//震荡总时间参数计算
float FunO3CalOSCParameterKV(float  _allV)
{
    if(_allV>=KVLEVEL1)
        kvosc = KVOSC1;
    else if(_allV>=KVLEVEL2)
...
//                   }
                ...
                case STATUS_AUTOV_RUN_F2: //第一次计算
                    kp = FunO3CalParameter();  //0.4  kp = kh *kt kh是湿度  kt 是温度 ，根据温湿度查表而来
                    g_V = g_lenth*g_width*g_height/1000; //200 体积
                    kv= FunO3CalParameterKV(g_V);  //kv  1.4 根据体积查表而来
                ...
                case STATUS_AUTOV_RUN_F3: //根据计算时间，判断下一步干什么
                    ...
                    {
                       g_status = STATUS_AUTOV_RUN_F4; //等待1分钟
                    }
                    break;

                case STATUS_AUTOV_RUN_F4: //等待1分钟 ，再次计算
                    g_timerun++;
                   if(g_timerun>AUTOVCALTIME)
                   {
                      kp = FunO3CalParameter();
                      g_AutoVtime = g_AutoVtime - AUTOVCALTIME*kp; //T2 = T1 -60*KP
