...
uint8_t g_status =0,temp_sta=1;   //系统运行的所有状态切换
...
//---------------------------自动浓度模式 状态轮询------------------begin----------------------------// 
               ...
               case STATUS_AUTODEN_RUN_F4_ON://>=0
                   if((g_nh3den>O3LEVEL3)) FunCheckStatus();
                   g_timerun++;
                   g_Nh3LowEnterCount++;
               ...
               if(g_timerun>FANOFFTIME) //等待300S后 所有关闭
               {
                ...
                case STATUS_AUTODEN_WAIT : //待机模式
                    if(g_nh3den>O3LEVEL3)   //浓度升高时开始切换
                    {
                        FunCheckStatus();
                        FunFanOpen();
                    }
...
//---------------------------自动体积模式 状态轮询------------------begin----------------------------//  
                ...
                case STATUS_AUTOV_RUN_F2:
                    g_AutoVtime = kv*g_lenth*g_width*g_height/1000.0-120*FunO3CalParameter();
                    printf("first time =%f\r\n",g_AutoVtime);
                    g_status = STATUS_AUTOV_RUN_F3;
                    break;
                case STATUS_AUTOV_RUN_F3: //循环计算时间
                    g_AutoVtime = g_AutoVtime - 120*FunO3CalParameter();
                     printf("cal time =%f\r\n",g_AutoVtime);
                    if(g_AutoVtime<5)
                    {
                ...
                case STATUS_AUTOV_RUN_ON://震荡模式 开阶段
                   ...
                   if(g_AutoVEntercount>kc*g_lenth*g_width*g_height/1000.0)
                   {
                       g_timerun =0;
                       g_AutoVEntercount=0;
