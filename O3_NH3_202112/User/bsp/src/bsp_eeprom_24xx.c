...
void ee_ReadSaveData(void)
{
    ...
    if(eeResult[0] &0x01)  //长度已经设置
    {
        g_lenth = ((uint16_t)eeResult[1]<<8)+eeResult[2];
          ...
          printf("Read lenth = %d m\r\n",g_lenth/10);
          bsp_Diwen_Updatedata(0x0010,g_lenth);
        #endif 
    }
    if(eeResult[0] &0x02)  //宽度已经设置
    {
          g_width = ((uint16_t)eeResult[3]<<8)+eeResult[4];
            ...
            printf("Read width = %d m\r\n",g_width/10);
            bsp_Diwen_Updatedata(0x0011,g_width);
            #endif 
    ...
    if(eeResult[0] &0x04)  //高度已经设置
    {
        ...
        printf("Read height = %d m\r\n",g_height/10);
        bsp_Diwen_Updatedata(0x0012,g_height);
        #endif 

    }
    
    if(eeResult[0] &0x08)  //手动开启时间已经设置
    {
        g_mannualtime = ((uint16_t)eeResult[8]<<8)+eeResult[9];
        #ifdef USEDEBUG

        printf("Read mannual time = %d min\r\n",g_mannualtime/10);
        bsp_Diwen_Updatedata(0x0000,g_mannualtime);
        #endif 

    }
    
    if(eeResult[0] &0x10)  //循环开启时间已经设置
    {
        g_loopruntime = ((uint16_t)eeResult[10]<<8)+eeResult[11];
        #ifdef USEDEBUG

        printf("loop start  time = %d min\r\n",g_loopruntime/10);
        bsp_Diwen_Updatedata(0x0001,g_loopruntime);
        #endif 

    }
    
    if(eeResult[0] &0x20)  //循环停止时间已经设置
    {
        g_loopstoptime = ((uint16_t)eeResult[12]<<8)+eeResult[13];
        #ifdef USEDEBUG

        printf("loop stop  time = %d min\r\n",g_loopstoptime/10);
        bsp_Diwen_Updatedata(0x0002,g_loopstoptime);
        #endif 
