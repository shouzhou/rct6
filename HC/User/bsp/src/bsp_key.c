...
uint8_t bsp_Read485Address(void)//连接时 ON 在上面 ON表示闭合 接地 低电平
{
    uint8_t temp=0x00;
    if(GPIO_ReadInputDataBit(GPIOC,GPIO_Pin_0) == 0) //SEL0 -低电平 打到ON上 -表示最高位逻辑1
    {
       temp|= 0x08;//00001000
    }
 ...
 //   temp = (uint8_t)GPIO_ReadInputData(GPIOC)&0x0f;
    return temp;
}
