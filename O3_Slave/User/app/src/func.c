...
void FunSwitch(void)
{
    #ifdef USEDEBUG
    if(g_status != 26)
    printf("current status = %d  SLAVE NUM = %d\r\n",g_status,SADDR485);
    ...
    if((g_status>10)&&(g_status<STATUS_AUTOV_WAIT))
       ...
       if(g_status >0)
       {
           if(g_tVar.P21 == 0xA55A) //发送关机命令
           {
                    FunO3Control(0,1);
                    g_status = STATUS_ALL_STOP;
                    printf("system all stop!\r\n");
           }
       }
        switch(g_status)
        {
...
//                   }
                ...
                case STATUS_AUTOV_RUN_F2: //第一次计算
                    ...
                    g_AutoVtime = kv*g_lenth*g_width*g_height/1000.0-120*kp;
                    printf("kp = %f, kv=%f, the time is %f\r\n",kp,kv,g_AutoVtime);
                ...
                case STATUS_AUTOV_RUN_F3: //根据计算时间，判断下一步干什么
                    ...
                    if(g_AutoVtime<AUTOVCALTIME)
                    {
                       ...
                       FunO3Control(2,g_o3SwitchIndex);//开2个
                        g_touch = 12;
                        g_AutoOsctime = kc*g_lenth*g_width*g_height/1000.0;
                    }
                    else  //时间未到
                    {
                ...
                case STATUS_AUTOV_RUN_F4: //等待2分钟 ，再次计算
                   ...
                   {
                       kp = FunO3CalParameter();
                      g_AutoVtime = g_AutoVtime - AUTOVCALTIME*kp; //T2 = T1 -2*KP
                ...
                case STATUS_AUTOV_RUN_OFF://震荡模式 关阶段
                     ...
                     g_timerun++;
                     g_AutoVEntercount++;
                   if(g_timerun>koscoff*100000.0/(g_lenth*g_width*g_height/1000.0)) //震荡关时间与体积成反比
                   {
                      g_timerun =0;
